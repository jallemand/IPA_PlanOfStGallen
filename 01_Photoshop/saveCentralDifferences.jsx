function saveCentralDifferences() {
	// Using the open document...
	...
	if (inputPSB.split(".")[1] === "psb") {
		...
		var temp = inputPSB.split(".")[0];
		temp = temp.split("_");
		
		var outImagesPath = outputPath + "/" + temp[5] + "_images";
		if (Folder(outImagesPath).exists == false) {
			new Folder(outImagesPath).create()
		};  

		// Save to the output PSB
		savePSB(outFullPath);
			...
			if (layer.name == 'Layer 1') {
				...
				var idsetd = charIDToTypeID( "setd" );
					...
					var idT = charIDToTypeID( "T   " );
						...
						desc2.putEnumerated( idMd, idBlnM, idDfrn );
					var idLyr = charIDToTypeID( "Lyr " );
				...
				try {
					...
					var idDlt = charIDToTypeID( "Dlt " );
						...
						var idnull = charIDToTypeID( "null" );
							...
							var idChnl = charIDToTypeID( "Chnl" );
							var idOrdn = charIDToTypeID( "Chnl" );
							var idTrgt = charIDToTypeID( "Msk " );
		...
		for (var i = 0; i < numLayers; i++) {
			...
			if (layer.name != 'Layer 1') {
				...
				currentLayer.name = currentLayer.name.split(".")[0] + "_inside_diff"
			}
		}
		doc.layers[0].remove();
		doc.save();
